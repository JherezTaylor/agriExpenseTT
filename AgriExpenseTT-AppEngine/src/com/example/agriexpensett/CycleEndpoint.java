...
public class CycleEndpoint {
	...
	public CollectionResponse < Cycle > listCycle(@Nullable@Named("cursor") String cursorString, @Nullable@Named("limit") Integer limit) {
	...
	@SuppressWarnings("unchecked")@ApiMethod(name = "getMatchingCycles")
	public List < Cycle > getMatchingCyles(@Named("cropName") String cropName, @Named("start") Double start, @Named("end") Double end, @Named("area") String area) {
		...
		if (area.equals("Nationwide")) {
			mgr = getEntityManager();
			query = mgr.createQuery("SELECT FROM Cycle AS Cycle WHERE cropName = :x AND totalSpent >= :y AND totalSpent <= :z ORDER BY totalSpent ASC");
			query.setParameter("x", cropName.toUpperCase());
			query.setParameter("y", start);
			query.setParameter("z", end);
		} else {
			mgr = getEntityManager();
			query = mgr.createQuery("SELECT FROM Cycle AS Cycle WHERE cropName = :x AND totalSpent >= :y AND totalSpent <= :z AND county = :a ORDER BY totalSpent ASC");
			query.setParameter("x", cropName.toUpperCase());
			query.setParameter("y", start);
			query.setParameter("z", end);
